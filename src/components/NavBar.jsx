...
const Navbar = () => {
    ...
    return (
        <header className='w-full bg-white md:bg-transparent fixed top-0 left-0 right-0'>
            <nav className={`py-4 lg:px-14 px-4 ${isSticky ? "sticky top-0 left-0 right-0 border bg-white duration-300":""}`}> 
                <div className='flex justify-between items-center text-base gap-8'>
                    ...
                    <ul className='md:flex space-x-12 hidden'>
                        {
                            navItems.map(({ link, path }) => <NavLink className={({ isActive, isPending }) =>
                                isActive
                                  ? "active"
                                  : isPending
                                  ? "pending"
                                  : ""
                              } to={path} spy={true} smooth={true}
                             offset={-100} key={path} >{link}</NavLink>) 
                        }
                    </ul> 
                ...
                <div className='md:hidden'>
                    <button
                    onClick={toggleMenu}
                     className='text-neutralDGrey focus:outline-none focus:text-grey-500 cursor-pointer'>
                     {
           ...
           {
                            navItems.map(({ link, path }) => <NavLink to={path} spy={true} smooth={true}  
                             offset={-100} key={path} className='block text-base text-white
                             hover:text-brandPrimary first:font-medium'>{link}</NavLink>) 
            }
           </div>
